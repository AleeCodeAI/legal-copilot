...
from evals.evals_observability.synthesizer_evaluator_observability import SynthesizerEvaluatorObservability  
import json
import time
import uuid
import logging
...
class AnswerSynthesizerEvals(Logger):
    """
    A class for evaluating the answer synthesizer agent quality.
    """
    name: str = "AnswerSynthesizerEvals"
    color: str = Logger.PURPLE

    def __init__(self):
        self.settings = get_settings()
        self.prompt = PromptManager().get_answer_synthesizer_evals_prompt()
        self.obs = SynthesizerEvaluatorObservability()
        ...
        self.evals_data = Path(__file__).parents[3] / "data" / "evals_data" / "answer_synthesizer_evals_data.json"
        self.execution_result = Path(__file__).parent / "results" / "answer_synthesizer_execution_results.json"
        self.evals_report = Path(__file__).parent / "results" / "answer_synthesizer_evals_report.md"

        self.log("AnswerSynthesizerEvals initialized successfully.")

    def _load_evals_data(self):
        try:
            with open(self.evals_data, 'r', encoding='utf-8') as file:
                data = json.load(file)
    ...
    def _save_execution_results(self, results: list[dict]):
        try:
            with open(self.execution_result, 'w', encoding='utf-8') as file:
        ...
        for attempt in range(1, self.settings.groq.max_retries + 1):
            try:
                response = self.client.chat.completions.parse(
                    ...
                    response_format=AnswerSynthesizerEvalsSchema 
                )
                
                result: AnswerSynthesizerEvalsSchema = response.choices[0].message.parsed
    ...
    def _evaluate_single_item(self, item, session_id) -> dict:
        ...
        try:
            ...
            self.log(f"Evaluation complete. Verdict: {result.verdict}, Confidence: {result.confidence}")
...
if __name__ == "__main__":
    evaluator = AnswerSynthesizerEvals() 
